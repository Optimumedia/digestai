...
| upgrade | rewrites the digest of a story that turned out to matter (several independent sources, a high score, or readers engaging with it) with OpenRouter's free Nemotron 3 Ultra for the day's few most important stories and Cloudflare Workers AI (Nemotron 3 120B, on its free daily neurons) for the rest first, then the strongest provider that is ahead of its daily pace, with Mistral as a last resort for multi-source digests only, and writes it from the best three or four sources together where the story has them, so it says what the sources agree on and where they differ. Bounded per run and per day, never twice for the same material, and it never touches a story's dates |
...
MISTRAL_API_KEY=...                  # https://console.mistral.ai (Free plan, $10/month; capped at MISTRAL_MONTHLY_CAP_USD=9)
CLOUDFLARE_ACCOUNT_ID=...            # Workers AI (free plan, 10,000 neurons/day; capped at CLOUDFLARE_DAILY_NEURONS=9000)
CLOUDFLARE_AI_TOKEN=...
OPENROUTER_API_KEY=...               # https://openrouter.ai (free ":free" models only, 50 requests/day; capped at OPENROUTER_DAILY_REQUESTS=45)
```
