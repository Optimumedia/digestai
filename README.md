...
| upgrade | rewrites the digest of a story that turned out to matter (several independent sources, a high score, or readers engaging with it) with Cloudflare Workers AI (Nemotron 3 120B, on its free daily neurons) first, then the strongest provider that is ahead of its daily pace, with Mistral as a last resort for multi-source digests only, and writes it from the best three or four sources together where the story has them, so it says what the sources agree on and where they differ. Bounded per run and per day, never twice for the same material, and it never touches a story's dates |
